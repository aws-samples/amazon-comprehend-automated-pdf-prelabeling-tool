...
class S3Helper(object):
    ...
    def __init__(self, region=os.environ.get("AWS_REGION")):
        """
    ...
    def create_bucket(self, s3_bucket_name: str):
        """Run a boto3 API to create an S3 bucket."""
        if not self.validate_bucket_exists(s3_bucket_name):
    ...
    def get_object_response_from_s3(self, s3_url: str):
        ...
        bucket, path = S3Helper.bucket_key_from_s3_uri(s3_url)
    ...
    def get_object_content_from_s3(self, s3_url: str):
        ...
        return (
            self.get_object_response_from_s3(s3_url).get("Body").read().decode("utf-8")
        )
    ...
    def write_content(self, s3_path: str, content: Union[str, bytes]):
        ...
        bucket, key = S3Helper.bucket_key_from_s3_uri(s3_path)
    ...
    def copy_file(self, old_s3_path: str, new_s3_path: str):
        ...
        old_bucket_name, old_key = S3Helper.bucket_key_from_s3_uri(old_s3_path)
        new_bucket_name, new_key = S3Helper.bucket_key_from_s3_uri(new_s3_path)
        try:
            copy_source = {"Bucket": old_bucket_name, "Key": old_key}
            self.s3.meta.client.copy(copy_source, new_bucket_name, new_key)
    ...
    def upload_file(self, local_path: str, bucket: str, s3_key: str, extra_args=None):
    ...
    def upload_directory(
        self, local_dir_path: str, bucket_name: str, s3_path_prefix: str
    ):
        """Upload a directory to S3."""
        for local_path, s3_key, extra_args in self._collect_s3_upload_file_arguments(
            local_dir_path, s3_path_prefix
        ):
            self.upload_file(
    ...
    def upload_pd_csv(self, df, s3_path: str):
        local_file_name = "temp.csv"
        df.to_csv(local_file_name)
        bucket, key = self.bucket_key_from_s3_uri(s3_path)
        self.upload_file(local_file_name, bucket, key)
    ...
    def download_directory(
        ...
        s3_download_file_args = self._get_s3_download_file_args(
            s3_objects=self.get_objects_from_s3(
                bucket=bucket_name, filter_prefix=s3_path_prefix
            ),
    ...
    def write_json_dict(self, s3_path: str, dict: Dict):
        """
        Write the passed in list of dictionaries to the given local or s3 path as JSONL.

        :param s3_path: Path to write to. Accepts local filesystem and S3 paths
        :param dict: the dict to be stored
        :return:
        """
    ...
    def _collect_s3_upload_file_arguments(self, local_path: str, s3_path_prefix: str):
        ...
        for root, dirs, files in os.walk(local_path):
            for file in files:
                ...
                upload_file_arguments.append(
                    (
                        os.path.join(root, file),
                        f"{s3_path_prefix}/{root}/{file}",
                        extra_args,
                    )
                )
    ...
    @staticmethod
    def s3_uri_from_bucket_folder_file(Bucket: str, folder: str, file: str) -> str:
        """s3 URL from Bucket and folder and file"""
        key = folder + file
        return "s3://{}/{}".format(Bucket, key)
