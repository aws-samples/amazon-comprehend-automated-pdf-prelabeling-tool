...
def give_textract_file_name(output_s3_prefix, status, file_name, page_number):
    file_name_prefix = get_file_name_without_ending(file_name)
    if output_s3_prefix[-1]=="/":
        return output_s3_prefix+status+'/'+"amazon-textract-output/"+file_name_prefix+"_page_"+page_number+".json"
    else:
        return output_s3_prefix+'/'+status+'/'+"amazon-textract-output/"+file_name_prefix+"_page_"+page_number+".json"

def give_annotation_file_name(output_s3_prefix, status, file_name):
    file_name_prefix = get_file_name_without_ending(file_name)
    if output_s3_prefix[-1]=="/":
        return output_s3_prefix+status+'/'+file_name_prefix+"_comp_ann.json"
    else:
        return output_s3_prefix+"/"+status+'/'+file_name_prefix+"_comp_ann.json"
